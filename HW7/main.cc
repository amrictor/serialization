    ...
    int main() {
		...
		IFSerial failTest("doesNotExist");
		assert(failTest.fail());
		assert(!failTest);
		assert(!failTest.eof());
		...
		{
		const string str = "constTest";
		const OFSerial const_out(str);
		assert(!const_out.fail());
		assert(const_out);
		...
		const bool cb = true;
		out << cb;
		const string cS = "fuck";
		out << cS;
		
		assert(!out.fail());
		assert(out);
		
		const IFSerial const_in(str);
		assert(!const_in.fail());
		assert(const_in);
		assert(!const_in.eof());
        ...
        cout<<"Testing OFSerial\nWriting to file\n";
        {
            ...
            out << 3111222333;
            out << -4555666777;
            out << 9223372036854775807 << -9223372036854775807-1;
            out << 2147483648;
            out << -2147483649;
            out << short(32767);
            out << short(6);
            out << short(-10);
            out << short(0);
            out << short(-32768) << "This is HW2 test case."s;
