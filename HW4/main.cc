...
int main(/*int argc, char * const argv[]*/){
    ...
    oss.str("");
    serialize(4,oss);
    cout<<"serialize 4"<<oss.str()<<'\n';
    assert(oss.str() =="i\x04"s);

    oss.str("");
    serialize("Jack Applin"s, oss);
    assert(oss.str() == "\x53\x10\x0b\x4a\x61\x63\x6b\x20\x41\x70\x70\x6c\x69\x6e"s);
    ...
    try {
		...
		//ERROR CASES
//  	iss.str("\x37"s);
//  	unserialize(iss, b); //not a boolean fail
// 		
//  	iss.str("\x69\x20\x9a\x6e"s);
//  	unserialize(iss, s); //not a tagged short fail
// 		
//  	iss.str("\x73\x20\x9a\x6e"s);
//  	unserialize(iss, s); //too big for short fail
// 		
//  	iss.str("\x6c\x42\x4a\x13\xfe\xf1"s);
//  	unserialize(iss, i); //not a tagged int fail
// 
//  	iss.str("\x69\x42\x4a\x13\xfe\xf1"s);
//  	unserialize(iss, i); //too big for an int fail
// 		
//  	iss.str("\x73\x20\x9a\x6e"s);
//  	unserialize(iss, l); //not a tagged long fail
// 		
//  	iss.str("\x6c\x90\x00\x7f\xff\xff\xff\xff\xff\xff\xff"s);
// 		unserialize(iss, l); //too big for a long fail
// 		
// 		iss.str("\x69\x2b"s);
//  	unserialize(iss, c); //not a tagged char fail
// 		
//  	iss.str("\x73\x10\x16\x54\x68\x69\x73\x20\x69\x73\x20\x48\x57\x32\x20\x74\x65\x73\x74\x20\x63\x61\x73\x65\x2e"s);
//  	unserialize(iss, S); //not a tagged string fail
// 		
//  	iss.str("\x53\x10\x25\x54\x68\x69\x73\x20\x69\x73\x20\x48\x57\x32\x20\x74\x65\x73\x74\x20\x63\x61\x73\x65\x2e"s);
//  	unserialize(iss, S); //length too large string fail
// 		
//  	iss.str("\x69\x62\x4a\x13\xfe\xf1"s);
//  	unserialize(iss, i); //length too large number fail
